...
class ChoicesMeta(type):
    ALLOWED_TYPES = (int, float, str, bool, tuple, type(None))
    ...
    def __init__(cls, name, bases, dct):
        ...
        for key, val in dct.items():
            if key.startswith('__') or key == 'Meta' or not isinstance(val, ChoicesMeta.ALLOWED_TYPES):
                continue

            if val is None and bases:
                choice = filter(lambda x: x[1] == key, choices)[0]  # should have only one
